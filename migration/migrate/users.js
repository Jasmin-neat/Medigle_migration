...
exports.migrateUsers = function (srcConnection, dstConnection, mgConnection) {
  ...
  query.on("result", function (row) {
    ...
    dstConnection.query(
      "SELECT id FROM users WHERE facility_id = ? AND user_id = ?",
      [row.medigle_id, row.user_name],
...
exports.updateUsersByFacilityId = async function (
  srcConnection,
  dstConnection,
  mgConnection,
  epark_id
  ...
  try {
    ...
    const queryExecutionTime = queryEnd - queryStart;
    console.log(
      `Inserted ${users.length} users for facility_id ${medigle_id}.`
    );
    console.log(
      `Query execution time: ${(queryExecutionTime / 1000).toFixed(2)} seconds.`
    );
