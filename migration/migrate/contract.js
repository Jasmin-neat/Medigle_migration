...
exports.migrateContracts = function (
  srcConnection,
  dstConnection,
  mgConnection
) {
  ...
  tableCreateQuery +=
    ", contract_contact_name varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL";
  tableCreateQuery +=
    ", contract_contact_department varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL";
  ...
  var query = srcConnection.query(
    "SELECT medigle_facility.*, epark_facility.* FROM epark_facility INNER JOIN medigle_facility ON epark_facility.id = medigle_facility.epark_id"
  );
  ...
  query.on("end", function () {
    ...
    console.log(
      `Query execution time: ${(queryExecutionTime / 1000).toFixed(2)} seconds.`
    );
  });
};

exports.migrateSingleContract = function (
  srcConnection,
  dstConnection,
  mgConnection,
  epark_id
    ...
    dstConnection.query(
      ...
      function (error, result) {
        ...
        if (result.affectedRows === 0) {
          console.log(
            `No contract found with epark_id ${epark_id} in the destination database`
          );
          return;
        }
        ...
        console.log(
          `Query execution time: ${(queryExecutionTime / 1000).toFixed(
            2
          )} seconds.`
        );
      }
...
exports.getMedigleFacilityId = function (srcConnection, epark_id) {
  return new Promise((resolve, reject) => {
    ...
    srcConnection.query(selectQuery, [epark_id], function (error, results) {
      ...
      console.log(
        `Medigle facility id for epark_id ${epark_id} is: ${medigle_id}`
      );
      resolve(medigle_id);
    });
  });
